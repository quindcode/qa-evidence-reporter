...
export interface JiraClient {
   ...
   * como un comentario NUEVO de `issueKey` vía la API REST v3 de Jira
   * Cloud — nunca borra ni reemplaza comentarios anteriores (a diferencia
   * del `.zip` adjunto, que sí se reemplaza en cada publicación, ver
   * `attachReport`). Publicar varias veces sobre el mismo issue deja UN
   * comentario por cada publicación, a propósito: es el historial de
   * trazabilidad de la sesión (decisión de producto explícita — el
   * criterio de "solo el último importa" quedó reservado para el adjunto,
   * no para los comentarios). Nunca toca el campo "Description" del issue,
   * ni ningún comentario escrito a mano por una persona del equipo.
   */
