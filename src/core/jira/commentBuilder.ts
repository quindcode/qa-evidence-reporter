 ...
 * esta herramienta — le aclara a quien lea el issue que ese comentario es
 * automático (no algo que haya escrito una persona del equipo a mano).
 */
 ...
 * adjunto), seguido de un resumen con el % de aprobado/fallado/omitido y,
 * al final, `markerParagraph()` (ver `QA_SUMMARY_COMMENT_MARKER` arriba).
 *
 * A diferencia del `.zip` adjunto (que SÍ se reemplaza en cada
 * publicación): `JiraClient.addComment` nunca borra comentarios
 * anteriores — cada publicación deja su propio comentario, a propósito,
 * para que el issue conserve la trazabilidad completa de cada corrida
 * (decisión de producto explícita, ver `jiraClient.ts`).
 *
...
export function buildQaSummaryComment(state: SessionState): AdfDocument {
  ...
  content.push(markerParagraph());

  return { type: 'doc', version: 1, content };
}

function markerParagraph(): unknown {
  return {
    type: 'paragraph',
    content: [{ type: 'text', text: QA_SUMMARY_COMMENT_MARKER, marks: [{ type: 'em' }] }],
  };
}
