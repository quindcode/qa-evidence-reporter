...
/**
 * Texto fijo que cierra todo comentario generado por esta herramienta —
 * doble función: (1) explica al humano que lee el issue por qué el
 * comentario anterior desapareció, (2) le da a `JiraClient.addComment` (ver
 * `jiraClient.ts`, `deleteAutoGeneratedComments`) una forma segura de
 * identificar CUÁLES comentarios existentes son "nuestros" y pueden borrarse
 * antes de publicar el nuevo, sin arriesgarse a borrar un comentario que
 * haya escrito una persona del equipo.
 */
export const QA_SUMMARY_COMMENT_MARKER =
  'Generado automáticamente por QA Evidence Reporter — este comentario se reemplaza en cada publicación nueva.';
 ...
 *
 * Solo el ÚLTIMO de estos comentarios importa (a diferencia del adjunto,
 * que ya se reemplazaba desde antes): `JiraClient.addComment` borra
 * cualquier comentario anterior que lleve este mismo marcador antes de
 * publicar el nuevo, para que el issue no acumule un historial de
 * resúmenes cada vez más largo y difuso — decisión de producto explícita,
 * ver `jiraClient.ts`.
 *
