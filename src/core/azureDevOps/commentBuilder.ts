 ...
 * publicación): `AzureDevOpsClient.addComment` nunca borra comentarios
 * anteriores — cada publicación deja su propio comentario, a propósito,
 * para que el work item conserve la trazabilidad completa de cada corrida.
 *
 ...
 * esta herramienta — mismo criterio que `QA_SUMMARY_COMMENT_MARKER` en
 * `core/jira/commentBuilder.ts`: le aclara a quien lea el work item que ese
 * comentario es automático.
 */
...
export function buildQaSummaryCommentHtml(state: SessionState): string {
  ...
  parts.push(`<p><em>${QA_SUMMARY_COMMENT_MARKER}</em></p>`);
