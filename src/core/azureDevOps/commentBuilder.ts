 ...
 * mismo criterio que `QA_SUMMARY_COMMENT_MARKER` en
 * `core/jira/commentBuilder.ts`: explica al humano por qué el comentario
 * anterior desapareció, y le da a `AzureDevOpsClient.addComment` (ver
 * `azureDevOpsClient.ts`, `deleteAutoGeneratedComments`) una forma segura de
 * identificar cuáles comentarios existentes son "nuestros".
 */
...
export function buildQaSummaryCommentHtml(state: SessionState): string {
  ...
  // Solo el ÚLTIMO comentario importa — ver JSDoc de `QA_SUMMARY_COMMENT_MARKER`
  // y `deleteAutoGeneratedComments` en `azureDevOpsClient.ts`.
