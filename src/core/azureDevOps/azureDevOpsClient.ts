...
export interface AzureDevOpsClient {
   ...
   * `workItemId` vía la API REST de Azure DevOps — nunca borra ni
   * reemplaza comentarios anteriores (a diferencia del `.zip` adjunto, que
   * sí se reemplaza en cada publicación, ver `attachReport`). Publicar
   * varias veces sobre el mismo work item deja UN comentario por cada
   * publicación, a propósito: es el historial de trazabilidad de la
   * sesión. Nunca pisa nada que una persona del equipo haya escrito a
   * mano.
   */
