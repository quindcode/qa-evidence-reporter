...
export interface AzureDevOpsClient {
   ...
   * `workItemId` vía la API REST de Azure DevOps. Nunca pisa nada que una
   * persona del equipo haya escrito a mano.
   *
   * ANTES de publicar, busca los comentarios que ya tiene `workItemId` y
   * borra (best-effort) los que lleven el marcador
   * `QA_SUMMARY_COMMENT_MARKER` — los que generó esta misma herramienta en
   * una publicación anterior. Mismo criterio que el adjunto (que ya se
   * reemplazaba): publicar dos veces deja SOLO el comentario más reciente,
   * nunca un historial acumulado y difuso.
   */
...
export function createAzureDevOpsClient(
  ...
  /**
   * Busca los comentarios que ya tiene `workItemId` (vía `GET .../comments`)
   * y borra (best-effort) los que contengan `QA_SUMMARY_COMMENT_MARKER` en
   * su `text` — mismo criterio que `buildRemoveOpsForExistingAttachment`
   * arriba, pero filtrando por marcador de texto en vez de por nombre de
   * archivo. Un fallo al borrar un comentario en particular se ignora en
   * silencio (mejor un comentario viejo sin borrar que bloquear el publish
   * entero por una limpieza secundaria).
   */
  async function deleteAutoGeneratedComments(
    ...
    const listUrl = `${apiBaseUrl}/_apis/wit/workItems/${workItemId}/comments?api-version=${COMMENTS_API_VERSION}`;
    ...
    await assertOk(listResponse, workItemId);

    const data = (await listResponse.json()) as { comments?: Array<{ id: number; text?: string }> };
    const existing = (data.comments ?? []).filter(
      (comment) => typeof comment.text === 'string' && comment.text.includes(QA_SUMMARY_COMMENT_MARKER),
    );

    for (const comment of existing) {
      try {
        await fetchImpl(
          `${apiBaseUrl}/_apis/wit/workItems/${workItemId}/comments/${comment.id}?api-version=${COMMENTS_API_VERSION}`,
  ...
  async function addComment(workItemId: number, html: string): Promise<void> {
    ...
    await deleteAutoGeneratedComments(workItemId, apiBaseUrl, authHeader);
