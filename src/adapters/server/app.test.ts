...
describe('createApp (integración, sin puerto TCP real — ver Bash/curl para la prueba con socket real)', () => {
  ...
  describe('POST /api/report/publish-jira', () => {
    ...
    it('publica exitosamente -> 201 + {issueKey, issueUrl}, con la URL/body correctos hacia Jira', async () => {
      ...
      // 3) GET de chequeo de comentarios propios previos (ver
      // `deleteAutoGeneratedComments`, reemplaza en vez de acumular),
      // 4) POST del comentario.
      ...
      const [commentUrl, commentInit] = fetchMock.mock.calls[3] as [string, RequestInit];
  ...
  describe('POST /api/report/publish-azure-devops', () => {
    ...
    it('publica exitosamente -> 201 + {workItemId, workItemUrl}, subiendo el adjunto y agregando el comentario', async () => {
      ...
      // work item, 4) GET de chequeo de comentarios propios previos (ver
      // `deleteAutoGeneratedComments`, reemplaza en vez de acumular),
      // 5) POST del comentario.
