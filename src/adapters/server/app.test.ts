...
`;
  ...
  describe('POST /api/report/publish-jira', () => {
    ...
    it('publica exitosamente -> 201 + {issueKey, issueUrl}, con la URL/body correctos hacia Jira', async () => {
      ...
      // 1) GET de chequeo de adjuntos duplicados, 2) POST del adjunto,
      // 3) POST del comentario (los comentarios NUNCA se borran — se
      // acumulan a propósito, para trazabilidad; solo el adjunto se
      // reemplaza en cada publicación).
      ...
      // % sobre scenarios (1 scenario, "Login exitoso", que pasó) — misma
      // base de cálculo que el dashboard del reporte HTML, no sobre steps.
  ...
  describe('POST /api/report/publish-azure-devops', () => {
    ...
    it('publica exitosamente -> 201 + {workItemId, workItemUrl}, subiendo el adjunto y agregando el comentario', async () => {
      ...
      // 1) POST del adjunto, 2) GET de chequeo de duplicados, 3) PATCH del
      // work item, 4) POST del comentario (los comentarios NUNCA se
      // borran — se acumulan a propósito, para trazabilidad; solo el
      // adjunto se reemplaza en cada publicación).
      expect(fetchMock).toHaveBeenCalledTimes(4);

      const commentCall = fetchMock.mock.calls.find(
        ([url, init]: [string, RequestInit]) => url.includes('/comments') && init.method === 'POST',
      );
  ...
  describe('GET/PATCH /api/settings + POST /api/settings/*-token', () => {
    it('GET refleja projectName/jira/azureDevOps de la config, sin exponer ningún token', async () => {
      const app = createApp(
        await buildContext(projectRoot, {
          jira: { baseUrl: 'https://tuempresa.atlassian.net', email: 'qa@tuempresa.com' },
          azureDevOps: { organizationUrl: 'https://dev.azure.com/tuorg', project: 'Checkout' },
    ...
    it('PATCH actualiza projectName y se refleja de inmediato en GET /api/features (sin reiniciar el server)', async () => {
      const app = createApp(await buildContext(projectRoot));

      await request(app)
        .patch('/api/settings')
        .send({ projectName: 'Nombre Nuevo Desde El UI' })
        .expect(200);

      const features = await request(app).get('/api/features').expect(200);
      expect(features.body.projectName).toBe('Nombre Nuevo Desde El UI');
    });

    it('PATCH persiste en qa-config.json — sobrevive a un buildServerContext posterior (simulando un restart)', async () => {
      const configFilePath = join(projectRoot, 'qa-config.json');
      await writeFile(
        configFilePath,
        JSON.stringify(QaConfigSchema.parse({ projectName: 'Antes del restart' })),
        'utf-8',
      );
      const context = await buildContext(projectRoot);
      const app = createApp({ ...context, configFilePath });

      await request(app)
        .patch('/api/settings')
        .send({ jira: { baseUrl: 'https://tuempresa.atlassian.net', email: 'qa@tuempresa.com' } })
        .expect(200);

      const persisted = JSON.parse(await readFile(configFilePath, 'utf-8'));
      expect(persisted.jira).toEqual({
        baseUrl: 'https://tuempresa.atlassian.net',
        email: 'qa@tuempresa.com',
      });
    });

    it('PATCH con un baseUrl inválido -> 400 INVALID_REQUEST_BODY, sin persistir nada', async () => {
      const configFilePath = join(projectRoot, 'qa-config.json');
      const originalConfig = QaConfigSchema.parse({ projectName: 'Proyecto de prueba' });
      await writeFile(configFilePath, JSON.stringify(originalConfig), 'utf-8');
      const context = await buildContext(projectRoot);
      const app = createApp({ ...context, configFilePath });

      const response = await request(app)
        .patch('/api/settings')
        .send({ jira: { baseUrl: 'no-es-una-url' } })
        .expect(400);

      expect(response.body.error.code).toBe('INVALID_REQUEST_BODY');
      expect(JSON.parse(await readFile(configFilePath, 'utf-8')).jira.baseUrl).toBeNull();
    });

    it('POST /api/settings/jira-token deja tokenConfigured en true y nunca devuelve el valor del token', async () => {
      const app = createApp(await buildContext(projectRoot));

      const response = await request(app)
        .post('/api/settings/jira-token')
        .send({ token: 'un-token-secreto' })
        .expect(200);

      expect(response.body.jira.tokenConfigured).toBe(true);
      expect(JSON.stringify(response.body)).not.toContain('un-token-secreto');
    });

    it('POST /api/settings/jira-token con token: null limpia tokenConfigured', async () => {
      const app = createApp(await buildContext(projectRoot));
      await request(app).post('/api/settings/jira-token').send({ token: 'un-token' }).expect(200);
      ...
      expect(response.body.jira.tokenConfigured).toBe(false);
    });

    it('el token seteado por el UI NUNCA se persiste en qa-config.json', async () => {
      const configFilePath = join(projectRoot, 'qa-config.json');
      const originalConfig = QaConfigSchema.parse({ projectName: 'Proyecto de prueba' });
      await writeFile(configFilePath, JSON.stringify(originalConfig), 'utf-8');
      const context = await buildContext(projectRoot);
      ...
      await request(app)
        ...
        .expect(200);
      await request(app)
        .post('/api/settings/azure-token')
        .send({ token: 'un-pat-secreto' })
        .expect(200);

      const onDisk = await readFile(configFilePath, 'utf-8');
      expect(onDisk).not.toContain('un-token-secreto');
      expect(onDisk).not.toContain('un-pat-secreto');
    });
  });
});
